...
RUN apk --update add ca-certificates gnupg iptables squid wget \
 ...
 && wget -qO /usr/local/bin/gosu "https://github.com/tianon/gosu/releases/download/1.4/gosu-amd64" \
 && wget -qO /usr/local/bin/gosu.asc "https://github.com/tianon/gosu/releases/download/1.4/gosu-amd64.asc" \
 && gpg --verify /usr/local/bin/gosu.asc \
 && chmod +x /usr/local/bin/gosu \
 && apk del --purge gnupg wget \
